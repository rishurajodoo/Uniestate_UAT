...
class InstallmentWizard(models.TransientModel):
        ...
        for rec in self:
            ...
            if rec.is_tax_include == True:
                ...
                for record in order_liness:
                    if record.product_id.is_unit == True and record.is_debit == False and rec.milestone_id.is_post_cheque == False and rec.milestone_id.is_ejari == False and rec.milestone_id.is_debit == False and rec.milestone_id.is_renewal == False and rec.milestone_id.is_utility_charge == False and rec.milestone_id.is_chiller_charges == False:
                        # if record.is_unit == True:
                        tax = (rec.amount * record.tax_id.amount) / 100
                        rec.tax_amount = tax
                        break
                    elif record.product_id.is_unit == True and record.is_debit == True and rec.milestone_id.is_debit == True and rec.milestone_id.is_post_cheque == False and rec.milestone_id.is_ejari == False and rec.milestone_id.is_renewal == False and rec.milestone_id.is_utility_charge == False and rec.milestone_id.is_chiller_charges == False:
                        # if record.is_unit == True:
    ...
    def create_installments(self):
        ...
        #     End validations
        if self.is_ejari == False or self.is_post_cheque == False or self.is_renewal == False or self.is_utility_charge == False and self.is_chiller_charges == False:
            active_id.write({
    ...
    @api.onchange('is_chiller_charges')
    def onchange_is_chiller_charges(self):
        for rec in self:
            rec.is_chiller_charges = rec.milestone_id.is_chiller_charges
