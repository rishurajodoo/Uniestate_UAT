...
class AccountGeneralLedger(models.TransientModel):
    ...
    def view_report(self, option, tag):
        ...
        for account in account_ids:
            ...
            print("move_line_id IDS = ", account.id)
            move_line_list = []
            ...
            opening_debit = account.opening_debit or 0
            opening_credit = account.opening_credit or 0
    ...
    def get_filter_values(self, journal_id, date_range, options, analytic,
        ...
        for account in account_ids:
            move_line_id = move_line_ids.filtered(
                lambda x: x.account_id == account)
            print("move_line_id data = ",move_line_id)
            move_line_list = []
            for move_line in move_line_id:
                move_line_data = move_line.read(
                    ['date', 'name', 'move_name', 'debit', 'credit',
        ...
        if data:
            if report_action == 'dynamic_accounts_report.action_general_ledger':
                ...
                for account in data['account']:
                    ...
                    print("account Name is = ",data['data']['account_totals'][account]['opening_debit'])
                    account_record = self.env['account.account'].search([('id', '=', account_id)], limit=1)
                    print("The firs record data = ",account)
                    if account_record:
                        opening_debit = account_record.opening_debit or 0
                        opening_credit = account_record.opening_credit or 0
                        account_n = account_record.name

                        row += 1
                        sheet.write(row, col, 'Initial Balance', filter_head)
                        sheet.merge_range(row, col + 7, row, col + 8, data['data']['account_totals'][account]['opening_debit'], filter_head)
                        sheet.merge_range(row, col + 9, row, col + 10, data['data']['account_totals'][account]['opening_credit'], filter_head)
                        sheet.merge_range(row, col + 11, row, col + 12, data['data']['account_totals'][account]['opening_debit'] - data['data']['account_totals'][account]['opening_credit'], filter_head)
                        # print("Name............",data['data']['account_totals']'')
                        row += 1
                        ...
                        for rec in data['data'][account]:
                            print("The second record data = ", rec)
                            row += 1
                            ...
                            total_debit += rec[0]['debit']
                            total_credit += rec[0]['credit']

                        row += 1
                        sheet.write(row, col, 'Subtotal for ' + account, filter_head)
                        sheet.merge_range(row, col + 7, row, col + 8, total_debit, filter_head)
                        sheet.merge_range(row, col + 9, row, col + 10, total_credit, filter_head)
                        sheet.merge_range(row, col + 11, row, col + 12, total_debit - total_credit, filter_head)

                        row += 1
                        sheet.write(row, col, 'Total for ' + account, filter_head)
                        sheet.merge_range(row, col + 7, row, col + 8, total_debit + data['data']['account_totals'][account]['opening_debit'], filter_head)
                        sheet.merge_range(row, col + 9, row, col + 10, total_credit + data['data']['account_totals'][account]['opening_credit'], filter_head)
                        sheet.merge_range(row, col + 11, row, col + 12,
                                          (total_debit + opening_debit) - (total_credit + opening_credit), filter_head)
                        row += 1
                ...
                sheet.merge_range(row, col + 11, row, col + 12, float(data['grand_total']['total_debit']) - float(
                    data['grand_total']['total_credit']), filter_head)
